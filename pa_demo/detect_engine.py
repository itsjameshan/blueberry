...
def load_model(model_path, num_threads=None):
    ...
    try:
        ...
        _ort_session = ort.InferenceSession(model_path, sess_options=opts, providers=['CPUExecutionProvider'])
        _model_loaded = True
        return True
    except Exception:
        traceback.print_exc()
...
def postprocess(output, orig_w, orig_h, target_size, conf_threshold=0.5, iou_threshold=0.45, max_candidates=300):
    results = []
